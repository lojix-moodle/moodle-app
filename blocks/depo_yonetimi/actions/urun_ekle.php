...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $colors = $_POST['colors'] ?? []; // Renkler dizisi, boşsa boş dizi ata
    $sizes = $_POST['sizes'] ?? []; // Boyutlar dizisi, boşsa boş dizi ata
    $varyasyonlar = $_POST['varyasyon'] ?? []; // Varyasyonlar dizisi, boşsa boş dizi ata
...
?>
    ...
    <script>
        (function () {
            ...
            const generateRandomBarcodeBtn = document.getElementById('generate-random-barcode');
            ...
            // Renk kodları ve kontrast renkleri için yardımcı fonksiyonlar
            const colorMap = {
                'beyaz': '#FFFFFF',
                'mavi': '#0000FF',
                'siyah': '#000000',
            ...
            function getColorHex(colorName) {
                return colorMap[colorName.toLowerCase()] || '#CCCCCC'; // Varsayılan gri
            }
            ...
            // Raf ve Bölüm ilişkisi
            const rafBolumMap = {
                'Tişört': ['A1', 'A2', 'A3', 'A4', 'A5'],
                'Pantolon': ['B1', 'B2', 'B3', 'B4', 'B5'],
                'Ayakkabı': ['C1', 'C2', 'C3', 'C4', 'C5'],
                'Gömlek': ['D1', 'D2', 'D3', 'D4', 'D5'],
                'Elbise': ['E1', 'E2', 'E3', 'E4', 'E5'],
                'Ceket': ['F1', 'F2', 'F3', 'F4', 'F5'],
                'Aksesuar': ['G1', 'G2', 'G3', 'G4', 'G5'],
                'Çanta': ['H1', 'H2', 'H3', 'H4', 'H5'],
                'İç Giyim': ['I1', 'I2', 'I3', 'I4', 'I5']
            };

            bolumSelect.addEventListener('change', function() {
                const selectedBolum = this.value;
                rafSelect.innerHTML = '<option value="">-- Önce Bölüm Seçin --</option>'; // Önce temizle
                if (selectedBolum && rafBolumMap[selectedBolum]) {
                    rafBolumMap[selectedBolum].forEach(raf => {
                        const option = document.createElement('option');
                        option.value = raf;
                        option.textContent = raf;
            ...
            function displayVariants() {
                ...
                allVariants.forEach(variant => {
                    ...
                    // Correctly set the name for the array of variations
                    stockInput.name = `varyasyon[${variant.color.value}][${variant.size.value}]`;
                    stockInput.className = 'form-control'; // Added form-control class
                    stockInput.value = '0';
                    stockInput.min = '0';
                    stockInput.required = true;
                    stockInput.placeholder = 'Miktar girin';
                    ...
                    form.addEventListener('submit', function (event) {
                        if (!form.checkValidity()) {
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: 'Form Hatası!',
                                text: 'Lütfen tüm gerekli alanları doldurun ve geçerli bilgiler girin.',
                                confirmButtonText: 'Tamam',
            ...
            // Barkod oluşturma ve yazdırma işlevselliği
            generateRandomBarcodeBtn.addEventListener('click', function() {
                const randomNumber = Math.floor(100000000000 + Math.random() * 900000000000).toString(); // 12 haneli rastgele sayı
                barkodInput.value = randomNumber;
                generateBarcodeBtn.click(); // Rastgele oluşturulduktan sonra barkodu hemen görüntüle
            });

            generateBarcodeBtn.addEventListener('click', function() {
                const barcodeValue = barkodInput.value;
                if (barcodeValue) {
                    try {
                        JsBarcode("#barcode-svg", barcodeValue, {
                            format: "CODE128", // En yaygın barkod formatlarından biri
                            displayValue: true,
                            width: 2,
                            height: 60,
                            margin: 0,
                        ...
                        printBarcodeBtn.disabled = false; // Barkod oluşturulduysa yazdırma butonunu etkinleştir
                    } catch (e) {
                        Swal.fire({
                            icon: 'error',
                            title: 'Barkod Hatası',
                            text: 'Geçersiz barkod değeri. Lütfen geçerli bir barkod numarası girin.',
                            confirmButtonText: 'Tamam',
                            confirmButtonColor: '#dc3545'
                        });
                        barcodeSvg.innerHTML = ''; // Hata durumunda SVG'yi temizle
                        printBarcodeBtn.disabled = true;
                    }
                } else {
                    barcodeSvg.innerHTML = ''; // Değer yoksa SVG'yi temizle
                    printBarcodeBtn.disabled = true;
                    Swal.fire({
                        icon: 'warning',
                        title: 'Barkod Girilmedi',
                        text: 'Lütfen bir barkod numarası girin veya otomatik oluşturun.',
                        confirmButtonText: 'Tamam',
    ...
    icon: 'warning',
    title: 'Barkod Yok',
    text: 'Yazdırılacak bir barkod bulunamadı. Lütfen önce barkod oluşturun.',
    confirmButtonText: 'Tamam',
    confirmButtonColor: '#ffc107'
    });
    }
