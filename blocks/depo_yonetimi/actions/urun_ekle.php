...
printWindow.document.write('<svg id="print-barcode-svg"></svg>');
printWindow.document.write(`
<script>
    JsBarcode("#print-barcode-svg", "${barkodInput.value}", {
        format: "CODE128",
        lineColor: "#000",
        width: 2,
        height: 100,
...
// Varyasyon barkodları için gerekli fonksiyonlar
const varyasyonOlusturBtn = document.getElementById('varyasyonOlustur');
const varyasyonTablo = document.getElementById('varyasyonTablo');

// Varyasyon tablosu oluşturulduğunda barkodları ekle
if (varyasyonOlusturBtn) {
varyasyonOlusturBtn.addEventListener('click', function() {
...
if (!varyasyonTablo) return;

const anaBarkod = barkodInput.value.trim();
if (!anaBarkod) return;

const satirlar = varyasyonTablo.querySelectorAll('tr');
...
const variantCell = satir.querySelector('td:first-child');
if (!variantCell) return;

const variantText = variantCell.textContent.trim();
// Renk / Boyut formatındaki metni al
const parts = variantText.split('/');
if (parts.length < 2) return;

const renk = parts[0].trim();
const beden = parts[1].trim();

// Benzersiz varyasyon barkodu oluştur
const varyasyonBarkod = `${anaBarkod}-${String(index + 1).padStart(2, '0')}`;

// Barkod hücresi var mı kontrol et
let barkodHucresi = satir.querySelector('.varyasyon-barkod');

if (!barkodHucresi) {
// Stok miktarı hücresinden sonra barkod hücresi ekle
const miktarHucresi = satir.querySelector('td:nth-child(2)');
const yeniHucre = document.createElement('td');
yeniHucre.className = 'varyasyon-barkod';
satir.appendChild(yeniHucre);
barkodHucresi = yeniHucre;
}
...
const printBtn = barkodHucresi.querySelector('.print-varyasyon-barkod');
if (printBtn) {
...
// Sayfa yüklendiğinde ana barkod var ise görüntüle
if (barkodInput && barkodInput.value.trim() !== '') {
generateBarcode();
}
});
</script>
