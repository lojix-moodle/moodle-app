...
$PAGE->set_title(get_string('depo_duzenle', 'block_depo_yonetimi', 'Depo Düzenle'));
$PAGE->set_heading(get_string('depo_duzenle', 'block_depo_yonetimi', 'Depo Düzenle'));
...
$PAGE->navbar->add(get_string('depo_duzenle', 'block_depo_yonetimi', 'Depo Düzenle'));

// Depo verisini al
$depo = $DB->get_record('block_depo_yonetimi_depolar', ['id' => $depoid], '*', MUST_EXIST);

// Form sınıfı
class depo_duzenle_form extends moodleform {
    protected function definition() {
        global $DB;

        $mform = $this->_form;
        $depo = $this->_customdata['depo'];
        ...
        // Depo ID'sini gizli alan olarak ekle
        $mform->addElement('hidden', 'depoid', $depo->id);
...
// Form oluşturma
$form = new depo_duzenle_form(null, ['depo' => $depo]);

// Form başlangıç değerlerini ayarla
$form->set_data([
    'name' => $depo->name,
    'sorumluid' => $depo->sorumluid,
    'depoid' => $depo->id
]);
...
if ($form->is_cancelled()) {
    ...
    // Depoyu güncelle
    $update = new stdClass();
    $update->id = $data->depoid;
    $update->name = $data->name;
    $update->sorumluid = $data->sorumluid;
    $update->timemodified = time();

    $DB->update_record('block_depo_yonetimi_depolar', $update);

    // Başarı mesajı
    \core\notification::success(get_string('depo_guncellendi', 'block_depo_yonetimi', 'Depo başarıyla güncellendi.'));
    redirect(new moodle_url('/blocks/depo_yonetimi/index.php'));
}

// Çıktı
echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('depo_duzenle', 'block_depo_yonetimi', 'Depo Düzenle'));
